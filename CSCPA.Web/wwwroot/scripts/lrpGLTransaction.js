var lrpGLTransactionList = {
    instance: function () {
        return $("#GLTransactionList").dxDataGrid("instance");
    },
    refreshList: function () {
        var dataGrid = $("#GLTransactionList").dxDataGrid("instance");
        dataGrid.refresh();
    ...
    onSectionChnage: function (data) {
        ...
        //clearButton.option("disabled", !data.selectedRowsData.length);
    },
    onDeleteBtnClick: function () {
        ...
        swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    let dataGrid = $("#GLTransactionList").dxDataGrid("instance");
    ...
    onmassUpdateBtnClick: function () {
        ...
        let txtSalesTaxYesNo = $('#txtSalesTaxYesNo').dxSelectBox("instance").option('value')
        var datas = {};
        ...
        if (txtSalesTaxYesNo !== "") {
            datas["SalesTaxYesNoId"] = txtSalesTaxYesNo;
        }
        let dataGrid = $("#GLTransactionList").dxDataGrid("instance");
        $.when.apply($, dataGrid.getSelectedRowsData().map(function (data) {
    ...
    onClearBtnClick: function () {
        ...
        $('#txtDescription').dxTextBox("instance").option('value', "");
        $('#txtCheckNo').dxTextBox("instance").option('value', "");
        $('#txtSalesTaxAmount').dxTextBox("instance").option('value', "");
        $('#txtSalesTaxRate').dxTextBox("instance").option('value', "");
        $('#txtSalesTaxYesNo').dxSelectBox("instance").option('value', "");
        let dataGrid = $("#GLTransactionList").dxDataGrid("instance");
        dataGrid.deselectAll();
    ...
    saveState: function () {
        if ($('#Layouts').text() === "Add New") {
            ...
            const state = $("#GLTransactionList").dxDataGrid("instance").state();
            $.ajax({
    ...
    toolbar_preparing: function (e) {
        ...
        e.toolbarOptions.items.unshift({
            ...
            options: {
                ...
                disabled: true,
                onClick: function (e) {
                    lrpGLTransactionAddEdit.showModel(e, lrpGLTransactionList.refreshList());
                }
            }
        }, {
            location: "after",
            ...
            options: {
                ...
                disabled: true,
                onClick: function (e) {
                    lrpGLTransactionList.onDeleteBtnClick();
                }
            }
        });
    }
...
$(function () {
    $("#GLTransactionList > div > div.dx-datagrid-header-panel > div > div > div.dx-toolbar-after > div:nth-child(2) > div > div").attr("id", "Delete");
    $("#GLTransactionList > div > div.dx-datagrid-header-panel > div > div > div.dx-toolbar-after > div:nth-child(1)").hide();
    common.getlastLayout("glTransactionList");
})
var lrpGLTransactionAddEdit = {
    ...
    showModel: function (e, hideModelCallback) {
        ...
        $model.on("hidden.bs.modal", function (e) {
            if (hideModelCallback) {
                hideModelCallback(lrpGLTransactionAddEdit.hideModelCallbackData);
                return;
            }
        });
    ...
    save: function () {
        ...
        var validationResult = $("#LRPGLTransactionAddEditDevForm").dxForm("instance").validate();
        if (validationResult.isValid) {
            $.ajax({
                url: "/LRPGLTransaction/AddEdit",
                type: "POST",
                dataType: "json",
                data: common.dxFormData($("#LRPGLTransactionAddEditDevForm")),
                success: function (data) {
                ...
                beforeSend: function () {
                    common.showLoader($("#LRPGLTransactionAddEditDevForm"));
                },
                complete: function () {
                    common.hideLoader($("#LRPGLTransactionAddEditDevForm"));
                    lrpGLTransactionList.refreshList();
                }
            });
        }
    }
