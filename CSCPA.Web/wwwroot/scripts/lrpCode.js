var lrpCodeList = {
    instance: function () {
        return $("#LRPCodeList").dxDataGrid("instance");
    },
    refreshList: function () {
        $("#LRPCodeList").dxDataGrid("instance").refresh();
    },
    ...
    onDeleteBtnClick: function () {
        ...
        swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    let dataGrid = $("#LRPCodeList").dxDataGrid("instance");
    ...
    toolbar_preparing: function (e) {
        ...
        e.toolbarOptions.items.unshift({
            ...
            options: {
                icon: "plus",
                onClick: function (e) {
                    lrpCodeAddEdit.showModel(e, lrpCodeList.refreshList());
                }
            }
        }, {
            location: "after",
...
$(function () {
    $("#LRPCodeList > div > div.dx-datagrid-header-panel > div > div > div.dx-toolbar-after > div:nth-child(2) > div > div").attr("id", "Delete");
...
var lrpCodeAddEdit = {
    ...
    showModel: function (e, hideModelCallback) {
        ...
        $model.on("hidden.bs.modal", function (e) {
            if (hideModelCallback) {
                hideModelCallback(lrpCodeAddEdit.hideModelCallbackData);
                return;
            }
        });
        var $modelBody = $model.find(".modal-body");
        $modelBody.html("");
        $('.modal-title').text((e.row ? "Edit" : "Add") + " LRP Code")
        $model.modal("show");
        var url = "/LRPCode/";
        if (e.row)
    ...
    save: function () {
        ...
        var validationResult = $("#LRPCodeAddEditDevForm").dxForm("instance").validate();
        if (validationResult.isValid) {
            $.ajax({
                url: "/LRPCode/AddEdit",
                type: "POST",
                dataType: "json",
                data: common.dxFormData($("#LRPCodeAddEditDevForm")),
                ...
                beforeSend: function () {
                    common.showLoader($("#LRPCodeAddEditDevForm"));
                },
                complete: function () {
                    common.hideLoader($("#LRPCodeAddEditDevForm"));
                    lrpCodeList.refreshList();
                }
            });
        }
    }
