...
var countryList = {
    instance: function () {
        return $("#CountryList").dxDataGrid("instance");
    },
    refreshList: function () {
        $("#CountryList").dxDataGrid("instance").refresh();
    },
    ...
    onDeleteBtnClick: function () {
        ...
        swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    let dataGrid = $("#CountryList").dxDataGrid("instance");
    ...
    toolbar_preparing: function (e) {
        ...
        e.toolbarOptions.items.unshift({
            ...
            options: {
                icon: "plus",
                onClick: function (e) {
                    countryAddEdit.showModel(e, countryList.refreshList());
                }
            }
        }, {
                location: "after",
...
var countryAddEdit = {
    ...
    showModel: function (e, hideModelCallback) {
        ...
        $model.on("hidden.bs.modal", function (e) {
            if (hideModelCallback) {
                hideModelCallback(countryAddEdit.hideModelCallbackData);
                return;
            }
        });
        var $modelBody = $model.find(".modal-body");
        $modelBody.html("");
        $('.modal-title').text((e.row ? "Edit" : "Add") + " Country")
        $model.modal("show");
        var url = "/Country/";
        if (e.row)
    ...
    save: function () {
        ...
        var validationResult = $("#CountryAddEditDevForm").dxForm("instance").validate();
        if (validationResult.isValid) {
            $.ajax({
                url: "/Country/AddEdit",
                type: "POST",
                dataType: "json",
                data: common.dxFormData($("#CountryAddEditDevForm")),
                ...
                beforeSend: function () {
                    common.showLoader($("#CountryAddEditDevForm"));
                },
                complete: function () {
                    common.hideLoader($("#CountryAddEditDevForm"));
...
$(function () {
    $("#CountryList > div > div.dx-datagrid-header-panel > div > div > div.dx-toolbar-after > div:nth-child(2) > div > div").attr("id", "Delete");
    common.getlastLayout("countryList");
})
