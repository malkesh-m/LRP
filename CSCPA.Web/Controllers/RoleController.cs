...
namespace CSCPA.Web.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> GetExcel()
        {
            ...
            var results = await _RoleService.GetAll();
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", user + "_Role_Grid.xlsx");
                }
            }
        }
        ...
        {
            return PartialView("/Views/Role/_List.cshtml");
        }

        [HttpGet]
        public JsonResult List(DataSourceLoadOptions options)
        {
            return Json(_RoleService.GetPage(options));
        }
        ...
        {
            return PartialView("/Views/Role/_AddEdit.cshtml");
        }
        [Authorize("Permissions.Role.Edit")]
        [HttpGet]
        public async Task<IActionResult> Edit(Guid id)
        {
            return PartialView("/Views/Role/_AddEdit.cshtml", await _RoleService.Get(id));
        }
        ...
        {
            var result = await _RoleService.Update(key, values);
            if (result)
            {
                return Ok();
            }
        ...
        {
            var result = await _RoleService.Delete(key);
            if (result)
                return Json(new JsonResponse(ResponseType.Success, "Role" + GlobalConstant.Deleted));
            else
                return Json(new JsonResponse(ResponseType.Error, GlobalConstant.Error));
        }
        ...
        {
            return await _RoleService.GetLookup(options);
        }
    }
}
